...
class MainActivity : AppCompatActivity() {
    ...
    private val RunButton by lazy {  findViewById<Button>(R.id.btn_reroll) }
    ...
    private fun initAddButton() {
        addButton.setOnClickListener {
            when {
                ...
                pickNumberSet.size >= 5 -> showToast("숫자는 최대 5개까지 선택할 수 있다.")
                pickNumberSet.contains(numPick.value) -> showToast("이미 선택된 숫자다.")
                else -> {
    ...
    private fun setNumBack(number: Int, textView: TextView) {
        val background = when(number) {
            in 1..10 -> R.drawable.cicle_yellow
            in 11..20 -> R.drawable.cicle_blue
            in 21..30 -> R.drawable.cicle_red
            in 31..40 -> R.drawable.cicle_gray
            else -> R.drawable.cicle_green
        }
